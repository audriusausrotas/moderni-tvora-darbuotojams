    ...
    <div class="relative selct-none" :class="width ? `${width}` : 'w-48'">
      <div @click="isOpen = !isOpen"
        ...
        <div class="flex gap-3 capitalize">
          <NuxtImg v-if="name === 'verified'" :src="selectedValue === 'patvirtintas'
            ? '/icons/ellipseg.svg'
            : '/icons/ellipser.svg'
            " width="8" />
          {{ selectedValue }}
      ...
      <div v-if="isOpen"
        class="absolute left-0 z-50 flex flex-col w-[inherit] overflow-y-scroll border max-h-52 rounded-xl top-10 border-gray-light bg-gray-ultra-light">
        <div v-for="(value, index) in values" :key="index" @click="changeHandler(value)"
